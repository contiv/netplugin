...
GLOBAL OPTIONS:
   --consul-endpoints value, --consul value                 a comma-delimited list of netmaster consul endpoints, ignored when etcd-endpoints is set [$CONTIV_NETMASTER_CONSUL_ENDPOINTS]
   --etcd-endpoints value, --etcd value                     a comma-delimited list of netmaster etcd endpoints [$CONTIV_NETMASTER_ETCD_ENDPOINTS]
   --external-address value, --listen-url value             set netmaster external address to listen on (default: "<current-ip-address>:9999") [$CONTIV_NETPLUGIN_EXTERNAL_ADDRESS]
   ...
   --infra value, --infra-type value                        set netmaster infra tyoe, options [aci, default] (default: "default") [$CONTIV_NETMASTER_PLUGIN_HOST]
   --interal-address value, --control-url value             set netmaster internal address, listen on the same address of external address when not set [$CONTIV_NETMASTER_INTERNAL_ADDRESS]
   ...
   --name value, --plugin-name value                        set netmaster plugin name for docker v2 plugin (default: "netplugin") [$CONTIV_NETMASTER_PLUGIN_HOST]
   ...
   --use-json-log, --json-log                               set netmaster log format to json [$CONTIV_NETMASTER_USE_JSON_LOG]
   --use-syslog, --syslog                                   set netmaster send log to syslog or not [$CONTIV_NETMASTER_USE_SYSLOG]
   --help, -h                                               show help
...
GLOBAL OPTIONS:
   --consul-endpoints value, --consul value                 a comma-delimited list of netplugin consul endpoints, ignored when etcd-endpoints is set [$CONTIV_NETPLUGIN_CONSUL_ENDPOINTS]
   ...
   --host value, --host-label value                         set netplugin host to identify itself (default: "netplugin-node1") [$CONTIV_NETPLUGIN_HOST]
